...
不传 `manual_steps` 时默认在 `statement` 停下；显式传空数组 `[]` 表示所有步骤都自动执行。只要传入 `manual_steps`，它就必须是字符串数组；非数组、非字符串元素或未知步骤名都会返回 `400`，并且不会保存草稿题目。
...
`patch` 可选，用于在确认当前步骤时保存用户修改，必须是对象。补丁字段和 `/api/problems/{problem_id}/edit` 相同；如果补丁让题目内容发生变化，会让旧审查报告、验证报告、导出目录和 ZIP 失效。响应会包含 `changed`、`reports_invalidated` 和 `package_invalidated`。
