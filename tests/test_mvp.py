...
class AlgorithmQuestionMVPTest(unittest.TestCase):
    ...
    def test_export_package_replaces_directory_obstacle_with_package_file(self) -> None:
        problem = generate_problem(ProblemRequest(topic="array", use_llm=False))
        review = review_problem(problem)
        validation = validate_problem(problem, rounds=3)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            package_dir = root / problem.id
            package_dir.mkdir()
            package_file = package_dir / "problem.md"
            package_file.mkdir()
            (package_file / "stale.txt").write_text("stale", encoding="utf-8")

            export_problem_package(problem, root, validation, review)

            self.assertTrue(package_file.is_file())
            self.assertIn(problem.title, package_file.read_text(encoding="utf-8"))
    ...
    def test_reports_endpoint_does_not_mark_package_file_as_exported(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (package_root / problem.id).write_text("not a package directory", encoding="utf-8")
            problem_store.save(problem)
    ...
    def test_review_flags_dangerous_builtin_assigned_alias(self) -> None:
        problem = generate_problem(ProblemRequest(topic="array", use_llm=False))
        problem.reference_solution = "write_file = open\nwrite_file('out.txt', 'w')\nprint('x')\n"
    ...
    def test_review_flags_dangerous_builtin_dict_get_lookup(self) -> None:
        problem = generate_problem(ProblemRequest(topic="array", use_llm=False))
        problem.reference_solution = "__builtins__.__dict__.get('open')('out.txt', 'w')\nprint('x')\n"
    ...
    def test_review_flags_dangerous_builtin_dict_pop_lookup(self) -> None:
        problem = generate_problem(ProblemRequest(topic="array", use_llm=False))
        problem.reference_solution = "__builtins__.__dict__.pop('open')('out.txt', 'w')\nprint('x')\n"
